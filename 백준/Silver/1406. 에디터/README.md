...
<p>커서는 기본적으로 문자열 제일 뒤에 위치한다. 그러므로 cursor는 문자열의 길이로 설정할 수 있다.</p>
<p>input 의 문자열을 리스트로 받고 또 다른 덱을 생성해서 커서 뒤로 연결리스트를 만드는 방식으로 문제를 해결했다.</p>
<p>L 인 경우 문자열의 제일 마지막 문자를 appendleft로 덱에 추가한다.</p>
<p>D 인 경우 덱의 첫 번쨰 문자를 popleft 하여 원래의 문자열리스트의 마지막 원소가 되도록 append 한다.</p>
<p>B 인 경우 문자열 리스트에서 마지막 원소를 pop연산으로 제거 한다.</p>
<p>P 인 경우 문자열 리스트 마지막 원소로 추가하는 append연산을 수행한다.</p>
<br/>
<p>문자열 리스트 제일 뒤에 커서를 새로운 덱을 연결시킴으로써 문제를 해결했다.</p>
